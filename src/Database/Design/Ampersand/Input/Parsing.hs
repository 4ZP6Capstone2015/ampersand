...
-- | Parse an Ampersand file and all transitive includes. Backward compatability. 
...
parseADL opts thingToParse = getGrT (parseADL' opts thingToParse)

-- | Parse an Ampersand file and all transitive includes
parseADL' ::  Options                -- ^ The options given through the command line
         -> Either FilePath MetaType   -- ^ The path of the file to be parsed OR the MetaType. In the latter case, the files will be taken from `allStaticFiles`
         -> GuardedIO P_Context  -- ^ The resulting context
parseADL' opts thingToParse =  
  foldl1 mplus $ map (\idir -> do 
    let filePath = idir </> filePath'
    (ctxt, filePaths) <- parseSingleADL opts useAllStaticFiles filePath
    ctxts <- parseADLs opts useAllStaticFiles [filePath] (map (idir </>) filePaths)
    return $ foldl mergeContexts ctxt ctxts) (nonEmpty ["."] $ iDirs opts) 
     where 
       (filePath', useAllStaticFiles) = 
         case thingToParse of
           Left fp        -> (fp            ,False)
...
parseADLs :: Options                    -- ^ The options given through the command line
          ...
          -> GuardedIO [P_Context]   -- ^ The resulting contexts
parseADLs _    _                 _               []        = return []
parseADLs opts useAllStaticFiles parsedFilePaths filePaths =
 do { let filePathsToParse = nub filePaths \\ parsedFilePaths
    ; ctxtNewFilePathss <- sequenceA $ map (parseSingleADL opts useAllStaticFiles) filePathsToParse
    ; let (ctxts, newFilesToParse) = unzip ctxtNewFilePathss
    ; ctxts' <- parseADLs opts useAllStaticFiles (parsedFilePaths ++ filePaths) $ concat newFilesToParse
    ; return $ ctxts ++ ctxts'
    }
...
parseSingleADL ::
    Options
 -> Bool   -- True iff the file is from FormalAmpersand files in `allStaticFiles`
 -> FilePath -> GuardedIO (P_Context, [FilePath])
parseSingleADL opts useAllStaticFiles filePath = GrT $ do
      verboseLn opts $ "Reading file " ++ filePath ++ if useAllStaticFiles then " (from within ampersand.exe)" else ""
