{-# LANGUAGE UndecidableInstances, TemplateHaskell, ViewPatterns, StandaloneDeriving #-}
...
data Population
  = Tablei { _tablePop :: M.Map [Name] [SQLiCell] } 
  | Vectori { _vectorPop :: [[SQLiCell]] } 

makePrisms ''Population 

data SQLiSt = SQLiSt 
  { _stPopulations :: M.Map Name Population    -- Tables to populations 
  , _stLocalNames  :: M.Map Name Population    -- Locally bounds names 
...
evalValueExpr :: (MonadError SQLiErr m, MonadState SQLiSt m) => ValueExpr -> m Population
evalValueExpr Star = 
  ...
  -- return $ Tablei . M.foldMapWithKey $ \n -> \case 
  --   Vectori 
-- (\n -> \case 
--                        Vectori p -> M.fromList $ zip <$> 


-- evalValueExpr v@(NumLit rawLit) = 
--   (pure.pure) <$> mayToErr (InvalidLiteral v) ( 
--      (SomeCell <$> (readMaybe rawLit :: Maybe Integer)) <|> 
--      (SomeCell <$> (readMaybe rawLit :: Maybe Double))
--      )
-- evalValueExpr (StringLit _ str _) = return [[ SomeCell str ]] 
