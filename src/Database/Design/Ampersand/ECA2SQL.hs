...
import Database.Design.Ampersand.FSpec.SQL (expr2SQL,prettySQLQuery)
...
--SQLprinter :: Expr2SQL -> Maybe ValueExpr -> Doc -- Maybe ValueExpr is the where clause
--SQLprinter (x:xs) = mapM Show x ++"\n"++mapM ShowS SQLprinter(xs)
